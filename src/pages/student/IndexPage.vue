<script setup lang="js">
	...
	const selectedCourse = computed(
		() => route.query.course || 'No Course Selected',
	);
	const selectedMajor = computed(
		() => route.query.majors || 'No Major Selected',
	);
	...
	const fetchSubjects = async () => {
		...
		try {
			...
			const userDocSnap = await getDoc(doc(db, 'users', 'u85qKyi6zeeyVz5sF6UbDhIaadv2'));
			...
			// }
	     const selectedCourse = userDocSnap.data().selectedCourse;
			const courseDocSnap = await getDoc(doc(db, 'Courses', selectedCourse));
			console.log(courseDocSnap.data(), selectedCourse);
	  //    if (!data) {
	  //      console.warn("selectedCourse is undefined in the user document.");
	  //      return;
	  //  }
		// 	const courseDocSnap = await getDoc(
		// 		doc(db, 'course', selectedCourse),
		// 	);
		} catch (error) {
