<script setup>
	...
	const filteredStudents = computed(() => {
		...
		return students.value.filter((student) => {
			...
			const matchesStatus =
				status === '' || (student.status || 'pending').toLowerCase() === status;
		...
		try {
			if (modalType.value === 'accept') {
				...
				const { data, error } = await useFetch(
					'https://temp-server-nitro.vercel.app/api/email',
					{
						method: 'POST',
						headers: {
